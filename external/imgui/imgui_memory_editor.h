...
// If you don't use the default font, use ImGui::PushFont()/PopFont() to switch to a mono-space
...
// - v0.24 (2018/06/02): changed DragInt("Rows" to use a %d data format (which is desirable
// since imgui 1.61).
...
// - v0.33 (2018/10/10): added OptShowOptions option to hide all the interactive option
// setting.
...
// - v0.40 (2020/10/04): fix misuse of ImGuiListClipper API, broke with Dear ImGui 1.79. made
// cursor position appears on left-side of edit box. option popup appears on mouse release. fix
// MSVC warnings where _CRT_SECURE_NO_WARNINGS wasn't working in recent versions.
// - v0.41 (2020/10/05): fix when using with keyboard/gamepad navigation enabled.
// - v0.42 (2020/10/14): fix for . character in ASCII view always being greyed out.
// - v0.43 (2021/03/12): added OptFooterExtraHeight to allow for custom drawing at the bottom
// of the editor [@leiradel]
...
//
// Todo/Bugs:
// - This is generally old/crappy code, it should work but isn't very good.. to be rewritten
// some day.
// - PageUp/PageDown are supported because we use _NoNav. This is a good test scenario for
// working out idioms of how to mix natural nav and our own...
// - Arrows are being sent to the InputText() about to disappear which for LeftArrow makes the
// text cursor appear at position 1 for one frame.
// - Using InputText() is awkward and maybe overkill here, consider implementing something
// custom.
...
struct MemoryEditor {
    ...
    bool OptShowOptions;     // = true   // display options button/context menu. when disabled,
                             // options will be locked unless you provide your own UI for them.
    bool OptShowDataPreview; // = false  // display a footer previewing the
                             // decimal/binary/hex/float representation of the currently
                             // selected bytes.
    ...
    int OptMidColsCount;   // = 8      // set to 0 to disable extra spacing between every
                           // mid-cols.
    int OptAddrDigitsCount; // = 0      // number of addr digits to display (default calculated
                            // based on maximum displayed addr).
    float OptFooterExtraHeight; // = 0      // space to reserve at the bottom of the widget to
                                // add custom widgets
    ...
    bool (*HighlightFn)(const ImU8* data, size_t off); //= 0      // optional handler to return
    // Highlight property (to support
    ...
    void CalcSizes(Sizes& s, size_t mem_size, size_t base_display_addr) {
        ...
        s.HexCellWidth = (float)(int)(s.GlyphWidth * 2.5f); // "FF " we include trailing space
                                                            // in the width to easily catch
                                                            // clicks everywhere
    ...
    void DrawContents(void* mem_data_void, size_t mem_size,
        ...
        // We begin into our scrolling region with the 'ImGuiWindowFlags_NoMove' in order to
        // prevent click from moving the window.
        // This is used as a facility since our main click detection code doesn't assign an
        ...
        if (DataEditingAddr != (size_t)-1) {
            ...
            // (because currently we can't change the scrolling while the window is being
            // rendered)
                        ...
                        struct UserData {
                            ...
                            static int Callback(ImGuiInputTextCallbackData* data) {
                                ...
                                if (data->SelectionStart == 0 &&
                                    ...
                                    // content pulled from underlying memory data
                                    // (this is a bit tricky, since InputText technically
                                    // "owns" the master copy of the buffer we edit it in
                                    // there)
        ...
        // Notify the main window of our ideal child content size (FIXME: we are missing an API
        // to get the contents size from the child)
